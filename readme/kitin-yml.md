...
### `chunk-send`
- #### `use-vanilla-chunk-sender`
- **默认值:** `false`
- **建议值:** `如果你的服务器网络为单线，不稳定，存在严重跨网QoS现象，那强烈建议设置此值为true` **弱网环境强烈建议设置此值**
- **说明:**
  - moonrise发包策略**完全没有任何负反馈机制**,这导致国内很多网络环境很差的服务器，moonrise依旧疯狂发包，导致客户端被TCP包淹没而掉线。
  - 恢复Vanilla发包器，不使用moonrise发包
  - 好处:可以利用Vanilla的ACK等机制，动态调整服务器的发包速率，对弱网环境极其有效
  - 副作用:由于moonrise与vanilla的逻辑差异原因，导致地形加载速率奇慢，不过对于网络质量差的服务器，这可能反而是正向效果
  - 由于Vanilla发包器运行在区域线程，这使得内存占用高于moonrise
...
- **说明:**
  - 是否使用安全种子。
  - 安全种子会使用高安全性的密码学哈希函数，通过 1024 位种子来生成所有矿石和结构，而不是原版使用的 64 位种子。这可以从计算层面保护结构种子，使种子破解几乎不可能。
- **警告:**
  - 安全种子将从根本上改变矿石与结构相对于原版的生成位置。
    该选项仅作用于新生成的区块，因此如果你希望完整使用该功能，必须创建一个新世界。
    *一旦启用该配置项，便无法再切换回原版生成方式，除非你预生成整个世界，否则新生成的区块将出现地*形割裂的问题。