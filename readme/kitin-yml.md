...
- **说明:**
  - 将超过指定高度的区域视为"虚空”
  - **背景:** 有时候你会发现服务器某些地方明明没有什么，但就是带宽占用很高，不要奇怪，因为某些神秘实体（如小火球 SmallFireball）异常飞向无限高空，导致带宽占用以及服务端性能无意义占用的问题
- #### `enabled`
- **默认值:** `true`
- **说明:**
  - 是否开启高空虚空检测功能
...
- **默认值:** `false`
- **说明:**
  - 是否需要让玩家进入高空时也受到虚空伤害，这或许对某些服务器有用?
- #### `log-warnings`
- **默认值:** `true`
- **说明:**
  - 当有实体因高度过高被清理时，是否在后台打印 `WARN` 警告