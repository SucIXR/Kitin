...
public class PearlChunkLoading {
    public static void saveEnderPearls(ServerPlayer player, ValueOutput output) {
        var userPearls = PearlSnapshot.PEARL_DATA_MAP.remove(player.getUUID());
        if (userPearls != null && !userPearls.isEmpty()) {
            ValueOutput.ValueOutputList valueOutputList = output.childrenList("ender_pearls");
            int count = 0;
            int maxLimit = me.sucixr.kitin.config.KitinConfig.pearlFixMaxSave;
            java.util.List<PearlSnapshot.SimplePearlData> sortedList = new java.util.ArrayList<>(userPearls.values());
            sortedList.sort((a, b) -> Long.compare(b.creationTime(), a.creationTime()));
            for (PearlSnapshot.SimplePearlData data : sortedList) {
                if (maxLimit!=-1) {
