...
import io.papermc.paper.SparksFly;import io.papermc.paper.threadedregions.RegionizedServer;
...
public class KitinWorldInfoProvider implements WorldInfoProvider {
    private final SparksFly plugin;
    private final Server server;

    public KitinWorldInfoProvider(@UnknownNullability SparksFly plugin) {
        this.plugin = plugin;
    ...
    static final class FoliaChunkInfo extends AbstractChunkInfo<EntityType> {
        ...
        FoliaChunkInfo(Chunk chunk, World world, SparksFly plugin) {
...
//            Executor executor = task -> scheduler.execute(plugin, world, getX(), getZ(), task);
            ...
            // Kitin start fix 不使用Bukkit API，直接使用NMS
